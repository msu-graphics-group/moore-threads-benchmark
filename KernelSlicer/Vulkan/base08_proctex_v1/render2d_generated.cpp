...
void ProcRender2D_Generated::UpdatePlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  const size_t maxAllowedSize = std::numeric_limits<uint32_t>::max();
  m_uboData.m_time = m_time;
  m_uboData.m_usedImplementations = m_usedImplementations;
  m_uboData.allProcTextures_size     = uint32_t( allProcTextures.size() );     assert( allProcTextures.size() < maxAllowedSize );
  m_uboData.allProcTextures_capacity = uint32_t( allProcTextures.capacity() ); assert( allProcTextures.capacity() < maxAllowedSize );
...
void ProcRender2D_Generated::ReadPlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  a_pCopyEngine->ReadBuffer(m_classDataBuffer, 0, &m_uboData, sizeof(m_uboData));
  m_time = m_uboData.m_time;
  m_usedImplementations = m_uboData.m_usedImplementations;
  allProcTextures.resize(m_uboData.allProcTextures_size);
  all_references.resize(m_uboData.all_references_size);
}

void ProcRender2D_Generated::UpdateVectorMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  if(allProcTextures.size() > 0)
  {
    a_pCopyEngine->UpdateBuffer(m_vdata.allProcTexturesBuffer      , 0, allProcTextures_vtable.data(), allProcTextures_vtable.size()*sizeof(unsigned)*2 );
    a_pCopyEngine->UpdateBuffer(m_vdata.allProcTextures_dataVBuffer, 0, allProcTextures_dataV.data(), allProcTextures_dataV.size());
    for(auto it = allProcTextures_obj_storage_offsets.begin(); it != allProcTextures_obj_storage_offsets.end(); ++it) 
    {
      size_t     offset = it->second;
      const auto& odata = allProcTextures_sorted[it->first];
      if(odata.size() != 0)
        a_pCopyEngine->UpdateBuffer(m_vdata.allProcTextures_dataSBuffer, offset, odata.data(), odata.size());
    }
  }
...
void ProcRender2D_Generated::UpdateTextureMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
}

void ProcRender2D_Generated::EvaluateTexturesCmd(int w, int h, uint32_t* outData, int a_branchMode)
{
  ...
  pcData.m_a_branchMode = a_branchMode;
  vkCmdPushConstants(m_currCmdBuffer, EvaluateTexturesLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(KernelArgsPC), &pcData);
  
  vkCmdBindPipeline(m_currCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, EvaluateTexturesPipeline);
  vkCmdDispatch    (m_currCmdBuffer, (sizeX + blockSizeX - 1) / blockSizeX, (sizeY + blockSizeY - 1) / blockSizeY, (sizeZ + blockSizeZ - 1) / blockSizeZ);
}


void ProcRender2D_Generated::copyKernelFloatCmd(uint32_t length)
{
...
void ProcRender2D_Generated::matMulTransposeCmd(uint32_t A_offset, uint32_t B_offset, uint32_t C_offset, uint32_t A_col_len, uint32_t B_col_len, uint32_t A_row_len)
{
...
VkBufferMemoryBarrier ProcRender2D_Generated::BarrierForClearFlags(VkBuffer a_buffer)
{
...
VkBufferMemoryBarrier ProcRender2D_Generated::BarrierForSingleBuffer(VkBuffer a_buffer)
{
  VkBufferMemoryBarrier bar = {};
  bar.sType               = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
  bar.pNext               = NULL;
...
void ProcRender2D_Generated::BarriersForSeveralBuffers(VkBuffer* a_inBuffers, VkBufferMemoryBarrier* a_outBarriers, uint32_t a_buffersNum)
{
...
void ProcRender2D_Generated::FractalCmd(VkCommandBuffer a_commandBuffer, int w, int h, uint32_t* outData, int a_branchMode)
{
  VkPipelineStageFlagBits prevStageBits = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT };
    auto before = std::chrono::high_resolution_clock::now();
  {vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, EvaluateTexturesLayout, 0, 1, &m_allGeneratedDS[0], 0, nullptr);
  EvaluateTexturesCmd(w, h, outData, a_branchMode);
...
void ProcRender2D_Generated::Fractal(int w, int h, uint32_t* outData, int a_branchMode)
{
  ...
  if(Fractal_local.needToClearOutput)
  ...
  VkBuffer outDataGPU = vk_utils::createBuffer(device, w*h*sizeof(uint32_t ), outFlags);
  buffers.push_back(outDataGPU);
  ...
  m_exTimeFractal.msAPIOverhead = std::chrono::duration_cast<std::chrono::microseconds>(afterCreateObjects - beforeCreateObjects).count()/1000.f;
  ...
  m_exTimeFractal.msAPIOverhead += std::chrono::duration_cast<std::chrono::microseconds>(afterInitBuffers - afterCreateObjects).count()/1000.f;

  auto beforeSetInOut = std::chrono::high_resolution_clock::now();
  this->SetVulkanInOutFor_Fractal(outDataGPU, 0, 0);

  // (3) copy input data to GPU
  //
  auto beforeCopy = std::chrono::high_resolution_clock::now();
  m_exTimeFractal.msAPIOverhead += std::chrono::duration_cast<std::chrono::microseconds>(beforeCopy - beforeSetInOut).count()/1000.f;
  auto afterCopy = std::chrono::high_resolution_clock::now();
  m_exTimeFractal.msCopyToGPU = std::chrono::duration_cast<std::chrono::microseconds>(afterCopy - beforeCopy).count()/1000.f;
  //
  m_exTimeFractal.msExecuteOnGPU = 0;
  ...
  //
  {
    ...
    FractalCmd(commandBuffer, w, h, outData, a_branchMode);
    vkEndCommandBuffer(commandBuffer);
    auto start = std::chrono::high_resolution_clock::now();
    vk_utils::executeCommandBufferNow(commandBuffer, computeQueue, device);
    vkFreeCommandBuffers(device, commandPool, 1, &commandBuffer);
    auto stop = std::chrono::high_resolution_clock::now();
    m_exTimeFractal.msExecuteOnGPU += std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count()/1000.f;
  }

  // (5) copy output data to CPU
  //
  auto beforeCopy2 = std::chrono::high_resolution_clock::now();
  pCopyHelper->ReadBuffer(outDataGPU, 0, outData, w*h*sizeof(uint32_t ));
  this->ReadPlainMembers(pCopyHelper);
  afterCopy2 = std::chrono::high_resolution_clock::now();
  m_exTimeFractal.msCopyFromGPU = std::chrono::duration_cast<std::chrono::microseconds>(afterCopy2 - beforeCopy2).count()/1000.f;

  // (6) free resources
  //
  vkDestroyBuffer(device, outDataGPU, nullptr);
  ...
  m_exTimeFractal.msAPIOverhead += std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - afterCopy2).count()/1000.f;
}



void ProcRender2D_Generated::GetExecutionTime(const char* a_funcName, float a_out[4])
{
  vk_utils::ExecTime res = {};
  if(std::string(a_funcName) == "Fractal" || std::string(a_funcName) == "FractalBlock")
    res = m_exTimeFractal;
