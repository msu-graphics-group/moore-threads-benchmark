...
/////////////////////////////////////////////////////////////////////////////////////////// UBO
class ProcRender2D_Generated : public ProcRender2D
{
public:

  ProcRender2D_Generated()
  {
  }
  ...
  virtual void SetVulkanInOutFor_Fractal(
    VkBuffer outDataBuffer,
    size_t   outDataOffset,
    uint32_t dummyArgument = 0)
  {
    Fractal_local.outDataBuffer = outDataBuffer;
    Fractal_local.outDataOffset = outDataOffset;
    UpdateAllGeneratedDescriptorSets_Fractal();
  }

  virtual ~ProcRender2D_Generated();
...
virtual void UpdateDeviceData(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)   {
  ...
  virtual void FractalCmd(VkCommandBuffer a_commandBuffer, int w, int h, uint32_t* outData, int a_branchMode);

  void Fractal(int w, int h, uint32_t* outData, int a_branchMode) override;

  inline vk_utils::ExecTime GetFractalExecutionTime() const { return m_exTimeFractal; }

  vk_utils::ExecTime m_exTimeFractal;
  ...
  virtual void EvaluateTexturesCmd(int w, int h, uint32_t* outData, int a_branchMode);
...
protected:
  ...
  virtual void UpdateAllGeneratedDescriptorSets_Fractal();
  ...
  struct Fractal_Data
  {
    VkBuffer outDataBuffer = VK_NULL_HANDLE;
  ...
  {
    VkBuffer allProcTexturesBuffer = VK_NULL_HANDLE;
    size_t   allProcTexturesOffset = 0;
    VkBuffer allProcTextures_dataSBuffer = VK_NULL_HANDLE;
    size_t   allProcTextures_dataSOffset = 0;
    VkBuffer allProcTextures_dataVBuffer = VK_NULL_HANDLE;
    size_t   allProcTextures_dataVOffset = 0;
    VkBuffer all_referencesBuffer = VK_NULL_HANDLE;
    size_t   all_referencesOffset = 0;
  } m_vdata;
  std::vector<LiteMath::uint2> allProcTextures_vtable;
  std::vector<uint8_t>         allProcTextures_dataV;
  std::unordered_map<uint32_t, std::vector<uint8_t> > allProcTextures_sorted;
  std::unordered_map<uint32_t, size_t>                allProcTextures_obj_storage_offsets;
  struct AllBufferReferences 
  {
    VkDeviceAddress YellowNoiseAddress;
    VkDeviceAddress Mandelbrot2DAddress;
    VkDeviceAddress Ocean2DAddress;
    VkDeviceAddress Voronoi2DAddress;
    VkDeviceAddress Perlin2DAddress;
    VkDeviceAddress Julia2DAddress;
    VkDeviceAddress Pseudeo3DMandelbrotAddress;
  };
  ...
  VkPipelineLayout      EvaluateTexturesLayout   = VK_NULL_HANDLE;
  VkPipeline            EvaluateTexturesPipeline = VK_NULL_HANDLE;
  VkDescriptorSetLayout EvaluateTexturesDSLayout = VK_NULL_HANDLE;
  VkDescriptorSetLayout CreateEvaluateTexturesDSLayout();
  virtual void InitKernel_EvaluateTextures(const char* a_filePath);
  ...
  ProcRender2D_Generated_UBO_Data m_uboData;
