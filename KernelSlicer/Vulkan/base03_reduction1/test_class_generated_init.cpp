...
{
  auto pObj = std::make_shared<Numbers_Generated>();
...
vk_utils::VulkanDeviceFeatures Numbers_Generated_ListRequiredDeviceFeatures()
{
  ...
  res.features2 = Numbers_Generated::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_1;
  return res;
}

void Numbers_Generated::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
...
VkBufferUsageFlags Numbers_Generated::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t Numbers_Generated::GetDefaultMaxTextures() const { return 256; }

void Numbers_Generated::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void Numbers_Generated::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void Numbers_Generated::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
  vkDestroyBuffer(m_device, m_classDataBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.tmpred04Buffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.tmpred14Buffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.tmpred24Buffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.tmpred34Buffer, nullptr);
...
void Numbers_Generated::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void Numbers_Generated::InitKernel_ArraySumm(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_generated/kernel1D_ArraySumm.comp.spv");
  ...
  shaderPath = AlterShaderPath("shaders_generated/kernel1D_ArraySumm_Reduction.comp.spv");
  MakeComputePipelineOnly(shaderPath.c_str(), "main", kspec, ArraySummDSLayout, ArraySummLayout, &ArraySummReductionPipeline);
}


void Numbers_Generated::InitKernels(const char* a_filePath)
{
  InitKernel_ArraySumm(a_filePath);
}

void Numbers_Generated::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
  ...
  {
    const size_t sizeOfBuffer = ComputeReductionAuxBufferElements(a_maxThreadsCount, REDUCTION_BLOCK_SIZE)*sizeof(float);
    m_vdata.tmpred14Buffer = vk_utils::createBuffer(m_device, sizeOfBuffer, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    allBuffersRef.push_back(m_vdata.tmpred14Buffer);
  }
  {
    const size_t sizeOfBuffer = ComputeReductionAuxBufferElements(a_maxThreadsCount, REDUCTION_BLOCK_SIZE)*sizeof(float);
    m_vdata.tmpred24Buffer = vk_utils::createBuffer(m_device, sizeOfBuffer, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    allBuffersRef.push_back(m_vdata.tmpred24Buffer);
  }
  {
    const size_t sizeOfBuffer = ComputeReductionAuxBufferElements(a_maxThreadsCount, REDUCTION_BLOCK_SIZE)*sizeof(float);
    m_vdata.tmpred34Buffer = vk_utils::createBuffer(m_device, sizeOfBuffer, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    allBuffersRef.push_back(m_vdata.tmpred34Buffer);
  }
...
void Numbers_Generated::ReserveEmptyVectors()
{
}

void Numbers_Generated::InitDeviceData()
{
...
void Numbers_Generated::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[Numbers_Generated::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
Numbers_Generated::MemLoc Numbers_Generated::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
Numbers_Generated::MemLoc Numbers_Generated::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "Numbers_Generated::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void Numbers_Generated::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void Numbers_Generated::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 Numbers_Generated::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
Numbers_Generated::MegaKernelIsEnabled Numbers_Generated::m_megaKernelFlags;
