...
public:
  ...
  nBody() {}
  void setParameters(size_t a_size, uint32_t seed, uint32_t iters) {
    m_seed = seed;
    m_iters = iters;
    m_bodies.resize(a_size);
  }
  virtual void perform(BodyState *out_bodies [[size("a_size")]], uint32_t a_size);
