...
{
  auto pObj = std::make_shared<nBody_Generated>();
...
vk_utils::VulkanDeviceFeatures nBody_Generated_ListRequiredDeviceFeatures()
{
  ...
  res.features2 = nBody_Generated::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_1;
  return res;
}

void nBody_Generated::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
...
VkBufferUsageFlags nBody_Generated::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t nBody_Generated::GetDefaultMaxTextures() const { return 256; }

void nBody_Generated::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void nBody_Generated::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void nBody_Generated::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
  vkDestroyBuffer(m_device, m_classDataBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.m_bodiesBuffer, nullptr);
...
nBody_Generated::~nBody_Generated()
{
  ...
  vkDestroyDescriptorSetLayout(m_device, GenerateBodiesDSLayout, nullptr);
  GenerateBodiesDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorSetLayout(m_device, UpdateVelocityDSLayout, nullptr);
  UpdateVelocityDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorSetLayout(m_device, UpdatePositionDSLayout, nullptr);
  UpdatePositionDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorPool(m_device, m_dsPool, NULL); m_dsPool = VK_NULL_HANDLE;
  DeleteDeviceData();
}

void nBody_Generated::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void nBody_Generated::InitKernel_GenerateBodies(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_generated/kernel1D_GenerateBodies.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  GenerateBodiesDSLayout = CreateGenerateBodiesDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, GenerateBodiesDSLayout, &GenerateBodiesLayout, &GenerateBodiesPipeline);
  }
  else
  {
    GenerateBodiesLayout   = nullptr;
    GenerateBodiesPipeline = nullptr;
  }
}

void nBody_Generated::InitKernel_UpdateVelocity(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_generated/kernel1D_UpdateVelocity.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  UpdateVelocityDSLayout = CreateUpdateVelocityDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, UpdateVelocityDSLayout, &UpdateVelocityLayout, &UpdateVelocityPipeline);
  }
  else
  {
    UpdateVelocityLayout   = nullptr;
    UpdateVelocityPipeline = nullptr;
  }
}

void nBody_Generated::InitKernel_UpdatePosition(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_generated/kernel1D_UpdatePosition.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  UpdatePositionDSLayout = CreateUpdatePositionDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, UpdatePositionDSLayout, &UpdatePositionLayout, &UpdatePositionPipeline);
  }
  else
  {
    UpdatePositionLayout   = nullptr;
    UpdatePositionPipeline = nullptr;
  }
}


void nBody_Generated::InitKernels(const char* a_filePath)
{
  InitKernel_GenerateBodies(a_filePath);
  InitKernel_UpdateVelocity(a_filePath);
  InitKernel_UpdatePosition(a_filePath);
  std::string servPath = AlterShaderPath("shaders_generated/z_memcpy.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void nBody_Generated::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
...
void nBody_Generated::ReserveEmptyVectors()
{
  if(m_bodies.capacity() == 0)
    m_bodies.reserve(4);
}

void nBody_Generated::InitDeviceData()
{
  ...
  m_vdata.m_bodiesBuffer = vk_utils::createBuffer(m_device, m_bodies.capacity()*sizeof(BodyState), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.m_bodiesBuffer);
...
void nBody_Generated::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[nBody_Generated::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
nBody_Generated::MemLoc nBody_Generated::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
nBody_Generated::MemLoc nBody_Generated::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "nBody_Generated::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void nBody_Generated::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void nBody_Generated::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 nBody_Generated::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
nBody_Generated::MegaKernelIsEnabled nBody_Generated::m_megaKernelFlags;
