...
const float LATTICE_STEP = 5.f;
const uint LATTICE_RES = 10;
const float SOFTENING_CONST = 1e-5f;
const float BOUNDARY = 60.0f;
const float MASS = 5;
const float CHARGE_MULT = 5e13;
const float ELECTRON_CHARGE = 1.60218e-19;
const float PERMETTIVITY = 8.85418782e-12;
struct RandomGenT
{
  uvec2 state;
...
vec3 xyz(const vec4 vec) {
  return vec3(vec.x, vec.y, vec.z);
}

vec4 randFloat4(vec4 min_value, vec4 max_value, const uint threadId) {
