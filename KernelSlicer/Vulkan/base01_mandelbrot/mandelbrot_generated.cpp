...
#include "mandelbrot_generated.h"
...
void Mandelbrot_Generated::UpdatePlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
...
void Mandelbrot_Generated::ReadPlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  a_pCopyEngine->ReadBuffer(m_classDataBuffer, 0, &m_uboData, sizeof(m_uboData));
  m_time = m_uboData.m_time;
}

void Mandelbrot_Generated::UpdateVectorMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
}


void Mandelbrot_Generated::UpdateTextureMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
}

void Mandelbrot_Generated::processCmd(int w, int h, uint32_t* outData)
{
  ...
  uint32_t sizeX  = uint32_t(w);
  uint32_t sizeY  = uint32_t(h);
  ...
  vkCmdPushConstants(m_currCmdBuffer, processLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(KernelArgsPC), &pcData);
  
  vkCmdBindPipeline(m_currCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, processPipeline);
  vkCmdDispatch    (m_currCmdBuffer, (sizeX + blockSizeX - 1) / blockSizeX, (sizeY + blockSizeY - 1) / blockSizeY, (sizeZ + blockSizeZ - 1) / blockSizeZ);
}


void Mandelbrot_Generated::copyKernelFloatCmd(uint32_t length)
{
...
void Mandelbrot_Generated::matMulTransposeCmd(uint32_t A_offset, uint32_t B_offset, uint32_t C_offset, uint32_t A_col_len, uint32_t B_col_len, uint32_t A_row_len)
{
...
VkBufferMemoryBarrier Mandelbrot_Generated::BarrierForClearFlags(VkBuffer a_buffer)
{
...
VkBufferMemoryBarrier Mandelbrot_Generated::BarrierForSingleBuffer(VkBuffer a_buffer)
{
  VkBufferMemoryBarrier bar = {};
  bar.sType               = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
  bar.pNext               = NULL;
...
void Mandelbrot_Generated::BarriersForSeveralBuffers(VkBuffer* a_inBuffers, VkBufferMemoryBarrier* a_outBarriers, uint32_t a_buffersNum)
{
...
void Mandelbrot_Generated::FractalCmd(VkCommandBuffer a_commandBuffer, int w, int h, uint32_t* outData)
{
  VkPipelineStageFlagBits prevStageBits = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT };
    auto before = std::chrono::high_resolution_clock::now();
  {vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, processLayout, 0, 1, &m_allGeneratedDS[0], 0, nullptr);
  processCmd(w, h, outData);
...
void Mandelbrot_Generated::Fractal(int w, int h, uint32_t* outData)
{
  ...
  this->SetVulkanInOutFor_Fractal(outDataGPU, 0);
  ...
  //
  {
    ...
    FractalCmd(commandBuffer, w, h, outData);
...
void Mandelbrot_Generated::GetExecutionTime(const char* a_funcName, float a_out[4])
{
