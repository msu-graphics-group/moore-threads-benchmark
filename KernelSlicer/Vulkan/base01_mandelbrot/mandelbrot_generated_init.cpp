...
{
  auto pObj = std::make_shared<Mandelbrot_Generated>();
...
vk_utils::VulkanDeviceFeatures Mandelbrot_Generated_ListRequiredDeviceFeatures()
{
  ...
  res.features2 = Mandelbrot_Generated::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_1;
  return res;
}

void Mandelbrot_Generated::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
...
VkBufferUsageFlags Mandelbrot_Generated::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t Mandelbrot_Generated::GetDefaultMaxTextures() const { return 256; }

void Mandelbrot_Generated::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void Mandelbrot_Generated::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void Mandelbrot_Generated::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
...
Mandelbrot_Generated::~Mandelbrot_Generated()
{
  ...
  vkDestroyDescriptorSetLayout(m_device, processDSLayout, nullptr);
  processDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorPool(m_device, m_dsPool, NULL); m_dsPool = VK_NULL_HANDLE;
  DeleteDeviceData();
}

void Mandelbrot_Generated::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void Mandelbrot_Generated::InitKernel_process(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_generated/kernel2D_process.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  processDSLayout = CreateprocessDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, processDSLayout, &processLayout, &processPipeline);
  }
  else
  {
    processLayout   = nullptr;
    processPipeline = nullptr;
  }
}


void Mandelbrot_Generated::InitKernels(const char* a_filePath)
{
  InitKernel_process(a_filePath);
}

void Mandelbrot_Generated::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
...
void Mandelbrot_Generated::ReserveEmptyVectors()
{
}

void Mandelbrot_Generated::InitDeviceData()
{
...
void Mandelbrot_Generated::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[Mandelbrot_Generated::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
Mandelbrot_Generated::MemLoc Mandelbrot_Generated::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
Mandelbrot_Generated::MemLoc Mandelbrot_Generated::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "Mandelbrot_Generated::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void Mandelbrot_Generated::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void Mandelbrot_Generated::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 Mandelbrot_Generated::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
Mandelbrot_Generated::MegaKernelIsEnabled Mandelbrot_Generated::m_megaKernelFlags;
