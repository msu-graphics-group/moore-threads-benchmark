...
struct Sorter_Generated_UBO_Data
{
  uint dummy_last;
};
/////////////////////////////////////////////////////////////////////////////////////////// UBO
class Sorter_Generated : public Sorter
{
public:
  ...
  virtual void SetVulkanInOutFor_Sort(
  ...
  {
    Sort_local.a_dataBuffer = a_dataBuffer;
    Sort_local.a_dataOffset = a_dataOffset;
    Sort_local.a_outExcBuffer = a_outExcBuffer;
    Sort_local.a_outExcOffset = a_outExcOffset;
    UpdateAllGeneratedDescriptorSets_Sort();
  }

  virtual ~Sorter_Generated();
...
virtual void UpdateDeviceData(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)   {
  ...
  virtual void SortCmd(VkCommandBuffer a_commandBuffer, const uint2* a_data, unsigned int a_size, uint2* a_outExc);

  void Sort(const uint2* a_data, unsigned int a_size, uint2* a_outExc) override;

  inline vk_utils::ExecTime GetSortExecutionTime() const { return m_exTimeSort; }

  vk_utils::ExecTime m_exTimeSort;
  ...
  virtual void CopyDataCmd(const uint2* a_in, uint2* a_out, unsigned int a_size);
...
protected:
  ...
  virtual void UpdateAllGeneratedDescriptorSets_Sort();
  ...
  VkPipelineLayout      CopyDataLayout   = VK_NULL_HANDLE;
  VkPipeline            CopyDataPipeline = VK_NULL_HANDLE;
  VkDescriptorSetLayout CopyDataDSLayout = VK_NULL_HANDLE;
  VkDescriptorSetLayout CreateCopyDataDSLayout();
  virtual void InitKernel_CopyData(const char* a_filePath);
  ...
  Sorter_Generated_UBO_Data m_uboData;
