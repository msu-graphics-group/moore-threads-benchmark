...
void Sorter_Generated::UpdatePlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  const size_t maxAllowedSize = std::numeric_limits<uint32_t>::max();
  a_pCopyEngine->UpdateBuffer(m_classDataBuffer, 0, &m_uboData, sizeof(m_uboData));
}

void Sorter_Generated::ReadPlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  a_pCopyEngine->ReadBuffer(m_classDataBuffer, 0, &m_uboData, sizeof(m_uboData));
}

void Sorter_Generated::UpdateVectorMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
}


void Sorter_Generated::UpdateTextureMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
}

void Sorter_Generated::CopyDataCmd(const uint2* a_in, uint2* a_out, unsigned int a_size)
{
  ...
  vkCmdPushConstants(m_currCmdBuffer, CopyDataLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(KernelArgsPC), &pcData);
  
  vkCmdBindPipeline(m_currCmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, CopyDataPipeline);
  vkCmdDispatch    (m_currCmdBuffer, (sizeX + blockSizeX - 1) / blockSizeX, (sizeY + blockSizeY - 1) / blockSizeY, (sizeZ + blockSizeZ - 1) / blockSizeZ);
}


void Sorter_Generated::copyKernelFloatCmd(uint32_t length)
{
...
void Sorter_Generated::matMulTransposeCmd(uint32_t A_offset, uint32_t B_offset, uint32_t C_offset, uint32_t A_col_len, uint32_t B_col_len, uint32_t A_row_len)
{
...
VkBufferMemoryBarrier Sorter_Generated::BarrierForClearFlags(VkBuffer a_buffer)
{
...
VkBufferMemoryBarrier Sorter_Generated::BarrierForSingleBuffer(VkBuffer a_buffer)
{
  VkBufferMemoryBarrier bar = {};
  bar.sType               = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
  bar.pNext               = NULL;
...
void Sorter_Generated::BarriersForSeveralBuffers(VkBuffer* a_inBuffers, VkBufferMemoryBarrier* a_outBarriers, uint32_t a_buffersNum)
{
...
void Sorter_Generated::SortCmd(VkCommandBuffer a_commandBuffer, const uint2* a_data, unsigned int a_size, uint2* a_outExc)
{
  VkPipelineStageFlagBits prevStageBits = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT };
    {vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, CopyDataLayout, 0, 1, &m_allGeneratedDS[0], 0, nullptr);
  CopyDataCmd(a_data, a_outExc, a_size);
  ...
  vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, m_sort_uvec2.bitonicPassLayout, 0, 1, &m_allGeneratedDS[1], 0, nullptr);
  m_sort_uvec2.BitonicSortCmd(m_currCmdBuffer, a_size, m_devProps.limits.maxComputeWorkGroupSize[0]);
...
void Sorter_Generated::Sort(const uint2* a_data, unsigned int a_size, uint2* a_outExc)
{
  ...
  if(Sort_local.needToClearOutput)
  ...
  VkBuffer a_dataGPU = vk_utils::createBuffer(device, a_size*sizeof(const uint2 ), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  buffers.push_back(a_dataGPU);
  VkBuffer a_outExcGPU = vk_utils::createBuffer(device, a_size*sizeof(uint2 ), outFlags);
  ...
  m_exTimeSort.msAPIOverhead = std::chrono::duration_cast<std::chrono::microseconds>(afterCreateObjects - beforeCreateObjects).count()/1000.f;
  ...
  m_exTimeSort.msAPIOverhead += std::chrono::duration_cast<std::chrono::microseconds>(afterInitBuffers - afterCreateObjects).count()/1000.f;

  auto beforeSetInOut = std::chrono::high_resolution_clock::now();
  this->SetVulkanInOutFor_Sort(a_dataGPU, 0, a_outExcGPU, 0);

  // (3) copy input data to GPU
  //
  auto beforeCopy = std::chrono::high_resolution_clock::now();
  m_exTimeSort.msAPIOverhead += std::chrono::duration_cast<std::chrono::microseconds>(beforeCopy - beforeSetInOut).count()/1000.f;
  pCopyHelper->UpdateBuffer(a_dataGPU, 0, a_data, a_size*sizeof(const uint2 ));
  auto afterCopy = std::chrono::high_resolution_clock::now();
  m_exTimeSort.msCopyToGPU = std::chrono::duration_cast<std::chrono::microseconds>(afterCopy - beforeCopy).count()/1000.f;
  //
  m_exTimeSort.msExecuteOnGPU = 0;
  ...
  //
  {
    ...
    SortCmd(commandBuffer, a_data, a_size, a_outExc);
    vkEndCommandBuffer(commandBuffer);
    auto start = std::chrono::high_resolution_clock::now();
    vk_utils::executeCommandBufferNow(commandBuffer, computeQueue, device);
    vkFreeCommandBuffers(device, commandPool, 1, &commandBuffer);
    auto stop = std::chrono::high_resolution_clock::now();
    m_exTimeSort.msExecuteOnGPU += std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count()/1000.f;
  }

  // (5) copy output data to CPU
  //
  auto beforeCopy2 = std::chrono::high_resolution_clock::now();
  pCopyHelper->ReadBuffer(a_outExcGPU, 0, a_outExc, a_size*sizeof(uint2 ));
  this->ReadPlainMembers(pCopyHelper);
  afterCopy2 = std::chrono::high_resolution_clock::now();
  m_exTimeSort.msCopyFromGPU = std::chrono::duration_cast<std::chrono::microseconds>(afterCopy2 - beforeCopy2).count()/1000.f;
  ...
  m_exTimeSort.msAPIOverhead += std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now() - afterCopy2).count()/1000.f;
}



void Sorter_Generated::GetExecutionTime(const char* a_funcName, float a_out[4])
{
  vk_utils::ExecTime res = {};
  if(std::string(a_funcName) == "Sort" || std::string(a_funcName) == "SortBlock")
    res = m_exTimeSort;
