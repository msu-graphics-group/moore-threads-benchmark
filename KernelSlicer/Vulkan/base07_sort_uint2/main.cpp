...
int main(int argc, const char** argv)
{
  ...
  std::vector<uint2> array    (size*size); // 1024
  ...
  pImpl->GetExecutionTime("Sort", timings);
  std::cout << "Sort(exec) = " << timings[0]              << " ms " << std::endl;
  std::cout << "Sort(copy) = " << timings[1] + timings[2] << " ms " << std::endl;
  std::cout << "Sort(ovrh) = " << timings[3]              << " ms " << std::endl;
