...
public:
  ...
  TestClass(int winWidth, int winHeight, int a_maxThreads = 1)
  {
  ...
  void PackXY(uint tidX, uint tidY, uint* out_pakedXY [[size("tidX", "tidY")]]);
  void CastSingleRay(uint tid, const uint* in_pakedXY [[size("tid")]], 
                               uint* out_color        [[size("tid")]]);

  void StupidPathTrace(uint tid, uint a_maxDepth, 
                       const uint* in_pakedXY         [[size("tid")]], 
