...
{
  auto pObj = std::make_shared<TestClass_Generated>(winWidth, winHeight, a_maxThreads);
...
vk_utils::VulkanDeviceFeatures TestClass_Generated_ListRequiredDeviceFeatures()
{
  ...
  res.features2 = TestClass_Generated::ListRequiredDeviceFeatures(res.extensionNames);
...
void TestClass_Generated::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
  vkDestroyBuffer(m_device, m_classDataBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.m_randomGensBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.spheresMaterialsBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.spheresPosRadiusBuffer, nullptr);
...
TestClass_Generated::~TestClass_Generated()
{
  ...
  vkDestroyDescriptorSetLayout(m_device, StupidPathTraceMegaDSLayout, nullptr);
  StupidPathTraceMegaDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorSetLayout(m_device, CastSingleRayMegaDSLayout, nullptr);
  CastSingleRayMegaDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorSetLayout(m_device, PackXYMegaDSLayout, nullptr);
...
void TestClass_Generated::InitKernel_StupidPathTraceMega(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_generated/StupidPathTraceMega.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  StupidPathTraceMegaDSLayout = CreateStupidPathTraceMegaDSLayout();
  if(m_megaKernelFlags.enableStupidPathTraceMega)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, StupidPathTraceMegaDSLayout, &StupidPathTraceMegaLayout, &StupidPathTraceMegaPipeline);
  }
  else
  {
    StupidPathTraceMegaLayout   = nullptr;
    StupidPathTraceMegaPipeline = nullptr;
  }
}

void TestClass_Generated::InitKernel_CastSingleRayMega(const char* a_filePath)
{
...
void TestClass_Generated::InitKernel_PackXYMega(const char* a_filePath)
{
...
{
  InitKernel_StupidPathTraceMega(a_filePath);
...
void TestClass_Generated::ReserveEmptyVectors()
{
  ...
  if(spheresMaterials.capacity() == 0)
    spheresMaterials.reserve(4);
  if(spheresPosRadius.capacity() == 0)
    spheresPosRadius.reserve(4);
}

void TestClass_Generated::InitDeviceData()
{
  ...
  m_vdata.m_randomGensBuffer = vk_utils::createBuffer(m_device, m_randomGens.capacity()*sizeof(RandomGen), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.m_randomGensBuffer);
  m_vdata.spheresMaterialsBuffer = vk_utils::createBuffer(m_device, spheresMaterials.capacity()*sizeof(SphereMaterial), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.spheresMaterialsBuffer);
  m_vdata.spheresPosRadiusBuffer = vk_utils::createBuffer(m_device, spheresPosRadius.capacity()*sizeof(float4), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.spheresPosRadiusBuffer);
...
TestClass_Generated::MemLoc TestClass_Generated::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
TestClass_Generated::MemLoc TestClass_Generated::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
